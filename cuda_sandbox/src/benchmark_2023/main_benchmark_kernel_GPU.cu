...
{       
    ...
    //     cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rows_Dn_NN_inv, cols_res, cols_Dn_NN_inv, &alpha_cublas, d_Dn_NN_inv, ld_Dn_NN_inv, d_b, ld_res, &beta_cublas, d_r_stack, ld_r_stack); //d_b = d_res
...
{   
    ...
    //     cublasDgemm(cublasH, CUBLAS_OP_N, CUBLAS_OP_N, rows_D_IN, cols_N_init, cols_D_IN, &alpha_cublas, d_D_IN, ld_D_IN, d_N_init, ld_N_init, &beta_cublas, d_Nbar_stack, ld_beta);

    //     // LU factorization
    //     cusolverDnDgetrf(cusolverH, cols_C_NN, cols_C_NN, d_C_NN, ld_C_NN, d_work, NULL, d_info);

    //     // Solving the final system
    //     cusolverDnDgetrs(cusolverH, CUBLAS_OP_N, cols_C_NN, 1, d_C_NN, ld_C_NN, NULL, d_Nbar_stack, ld_beta, d_info);
    //     // TO BENCHMARK: END
    //     auto end = std::chrono::high_resolution_clock::now();
    //     auto elapsed_seconds = std::chrono::duration_cast<std::chrono::duration<double>>(end - start);
